...
export const SCHEMA = [
    ...
    { key: 'FP_READ_SOURCE', type: 'enum', def: 'filesystem', options: ['filesystem', 'webdav'], group: 'Fingerprint detection', label: 'Read the files through',
      help: 'filesystem = read them where Plex sees them, through the debrid mounts: nothing to configure. webdav = ask InfiniDysk and decypharr for exact byte ranges instead, which needs the WebDAV password below. Measured 2026-09-22 on 64 s of audio: 34 MB downloaded via WebDAV against 663 MB through the mount, which also slows the mount for anyone streaming.' },
    { key: 'FP_SEEDLESS', type: 'bool', def: true, group: 'Fingerprint detection', label: 'Work out intros for shows nothing knows yet',
      help: 'When a season has no known intro at all (no Plex marker, nothing in TheIntroDB), compare two of its episodes to each other: the long stretch of audio they share near the start is the title sequence. Costs two reads of the first minutes once per season, then the rest of the season follows cheaply. Their stories differ, so this can only ever pick up repeated material.' },
